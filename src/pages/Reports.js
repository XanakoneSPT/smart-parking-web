...
function Reports() {
  ...
  const fetchMetrics = async () => {
    try {
      ...
      const totalRevenue = paymentsResponse.data.reduce((sum, payment) => sum + payment.so_tien, 0);
      const totalTransactions = paymentsResponse.data.length;

      // Fetch active parking sessions
      const parkingResponse = await apiService.get('api_users/lichsuravao/');
      const activeParking = parkingResponse.data.filter(session => !session.thoi_gian_ra).length;
      ...
      const revenueByDate = paymentsResponse.data.reduce((acc, payment) => {
      ...
      // Process data for parking usage chart
      const parkingByDate = parkingResponse.data.reduce((acc, session) => {
